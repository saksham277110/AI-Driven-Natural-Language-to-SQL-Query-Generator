...
def generate_sql_query(nl_query):
    """Converts a natural language query into an optimized SQL query using Euron API."""
    schema = get_limited_schema()
    schema_text = "\n".join([
    ...
    headers = {
        "Authorization": test_key,  # put your key
    ...
    try:
        ...
        raw_sql_query = response_json["choices"][0]["message"]["content"].strip()
        return clean_sql_output(raw_sql_query)
